 ...
 */
package io.github.reflxction.snowy.util;
...
public class Paginator extends Menu {
    ...
    Paginator(EventWaiter waiter, Set<User> users, Set<Role> roles, long timeout, TimeUnit unit,
              ...
              boolean wrapPageEnds, String leftText, String rightText, boolean allowTextInput, List<Field> fields) {
        ...
        this.strings = items;
        if (strings.size() == 0)
            this.pages = (int) Math.ceil((double) fields.size() / itemsPerPage);
        else
    ...
    private Message renderPage(int pageNum) {
        ...
        int end;
        if (strings.size() != 0)
            end = strings.size() < pageNum * itemsPerPage ? strings.size() : pageNum * itemsPerPage;
        else
            end = fields.size() < pageNum * itemsPerPage ? fields.size() : pageNum * itemsPerPage;
        if (columns == 1) {
            ...
            for (int i = start; i < end; i++) {
                if (strings.size() != 0)
                    ...
                    ebuilder.addField(fields.get(i));
            }
            if (strings.size() != 0)
        ...
        //fields.forEach(ebuilder::addField);
    ...
    public static class Builder extends Menu.Builder<Builder, Paginator> {
        ...
        public Paginator build() {
            ...
            Checks.check(!strings.isEmpty() || !fields.isEmpty(), "Must include at least one item or field to paginate");
            ...
            return new Paginator(waiter, users, roles, timeout, unit, color, text, finalAction,
                    ...
                    bulkSkipNumber, wrapPageEnds, textToLeft, textToRight, allowTextInput, fields);
        }
